...
for i = 2:length(pos)
    ...
    %corvel = maxcorvel(jarda,raio_local(i)); %maximum corner velocity
    ...
    Fx = min(sqrt(1 - (Fy/tiresy_lim)^2)*tiresx_lim, motor_lim);
    accx(i) = (Fx-Fd(i)-Fz(i)*0.01)/mass;
    accy(i) = Fy/mass;

    time(i) = d(i)/vel(i);
    elapsed_time(i) = sum(time) + time(i);
end
newcheckeredVel = vel(end) + accx(end)*time(end);
if newcheckeredVel == checkeredVel
    condition = false;
else
    checkeredVel = newcheckeredVel;
end
clear i j k
end
%% Reverse Simulation (com travagens)
revvel = zeros(seg,1); revvel(end) = 80.4444;
revFd = zeros(seg,1); revFd(end) = 0.5*ro*SCX*revvel(end)^2;
revFl = zeros(seg,1); revFl(end) = 0.5*ro*SCZ*revvel(end)^2;
revFz = zeros(seg,1); revFz(end) = weight + revFl(end);
revaccx = zeros(seg,1); revaccx(end) = (((mx*(revFz(end)/4) + bx)*revFz(end))+revFd(end)+revFz(end)*0.01)/mass;
revaccy = zeros(seg,1); revaccy(end) = 0;
revtime = zeros(seg,1); revtime(end) = (-revvel(end)+sqrt((revvel(end)^2)+2*revaccx(end)))/(revaccx(end));
revelapsed_time = zeros(seg,1); revelapsed_time(end) = revtime(end);

for j = length(pos)-1:-1:1
    revcorvel = maxcorvel(raio_local(j)); %maximum corner velocity
    %revcorvel = maxcorvel(jarda,raio_local(i)); %maximum corner velocity
    ...
    Fy = min(tiresy_lim, mass*(revvel(j)^2)/raio_local(j));
    Fx = sqrt(1 - (Fy/tiresy_lim)^2)*tiresx_lim;
    revaccx(j) = (Fx+revFd(j)+revFz(j)*0.01)/mass;
    revaccy(j) = Fy/mass;
...
%     newrevFd(j) = revFd(i);
%     newrevFz(j) = revFz(i);
%     newrevaccx(j) = revaccx(j);
%     newrevaccy(j) = revaccy(j);
%     j = j+1;
% end
...
for i = 1:seg
    if Vel_PM(i) == vel(i)
        ...
        EnergyHC_PM(i) = 2.778*10^(-7) * (Fz_PM(i)) * sin(phi(i)) * d(i);
...
% clearvars -except Vel_PM Time_PM Deltatime_PM Fd_PM Fz_PM AccX_PM AccY_PM EnergyRR_PM EnergyHC_PM EnergyAero_PM EnergyLI_PM MechanicalEnergy MechanicalEnergy_PM